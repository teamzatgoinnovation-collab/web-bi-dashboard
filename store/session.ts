...
export type ConnectionConfig = { baseUrl: string };
...
const defaultBaseUrl =
  process.env.NEXT_PUBLIC_FRAPPE_BASE_URL || "https://demo.zatgo.online";
...
export const useSessionStore = create<SessionState>()(
  persist(
    (set) => ({
      ...
      setSession: ({ connected, user = null, fullName = null, error = null, baseUrl }) =>
        set((state) => ({
          ...
          fullName,
          lastError: error,
          connection: baseUrl ? { ...state.connection, baseUrl } : state.connection,
        })),
      clearSession: () =>
        set((state) => ({
          connected: false,
    ...
    {
      name: "zatgo-bi-dashboard-session",
      partialize: (s) => ({
